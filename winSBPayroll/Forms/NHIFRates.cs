...
namespace winSBPayroll.Forms
{
    public partial class NHIFRates : Form
    {
        ...
        public NHIFRates(string user, string Conn)
        {
            InitializeComponent();
        ...
        private void NHIFRates_Load(object sender, EventArgs e)
        {
            try
            {
                dataGridViewNHIF.AutoGenerateColumns = false;
                bindingSourceNHIF.DataSource = de.NHIFTable();
                this.dataGridViewNHIF.SelectionMode = DataGridViewSelectionMode.FullRowSelect;
                dataGridViewNHIF.DataSource = bindingSourceNHIF;
                groupBox2.Text = bindingSourceNHIF.Count.ToString();
            }
            catch (Exception ex)
            {
        ...
        private void btnUpload_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                UploadNhifRates(true, strFileName, _User);
        ...
        private void UploadNhifRates(bool Overwrite, string strFileName, string User)
        {
            string query = null;
            ...
            using (var bulkCopy = new SqlBulkCopy(destinationConnection))
            {
                ...
                if (Overwrite)
                {
                    try
                    {
                        string deleteQuery = "Delete from NHIFRates";
