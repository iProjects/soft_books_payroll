    ...
    class PAYEExcelBuilder
    {
       ...
       PAYEModel payeModel;
        ...
        private void AddDocHeader(ref int row, ref int col)
        {
            ...
            document.createHeaders(row, col, payeModel.employeraddress, cellrangeaddr1, cellrangeaddr1, 0, "WHITE", true, 10, "n");

            row++;
            cellrangeaddr1 = document.IntAlpha(col) + row;
            document.createHeaders(row, col, "PAYEE", cellrangeaddr1, cellrangeaddr1, 0, "WHITE", true, 10, "n");

            row++;
            cellrangeaddr1 = document.IntAlpha(col) + row;
            document.createHeaders(row, col, payeModel.ReportName, cellrangeaddr1, cellrangeaddr1, 0, "WHITE", true, 10, "n");
        ...
        private void AddDocBody(ref int row, ref int col)
        {
            ...
            //Add table detail
            foreach (var d in payeModel.PAYEItemList)
            {
        ...
        private void AddBodytableHeaders(ref int row, ref int col)
        {
            ...
            col++;
            cellrangeaddr1 = document.IntAlpha(col) + row;
            document.createHeaders(row, col, "PIN NO", cellrangeaddr1, cellrangeaddr1, 0, "WHITE", true, 10, "n");
        ...
        private void AddDocBodyTableTotals(ref int row, ref int col)
        {
            ...
            col++;
            cellrangeaddr1 = document.IntAlpha(col) + row;
            document.createHeaders(row, col, payeModel.TotalPAYE.ToString("#,##0"), cellrangeaddr1, cellrangeaddr1, 0, "WHITE", true, 10, "n");
        }
