...
{
    public class BankTransferPDFBuilder3
    {
        BankTransferReportModel2 _ViewModel;
        Document document;
        string Message;
        string sFilePDF;
        ...
        //constructor
        public BankTransferPDFBuilder3(BankTransferReportModel2 bbtransfermodel, string FileName, EventHandler<notificationmessageEventArgs> notificationmessageEventname)
        {
            ...
            _notificationmessageEventname.Invoke(this, new notificationmessageEventArgs("Constructed BankTransferPDFBuilder3", TAG));
        ...
        private void AddBankTransfers()
        {
            try
            {
                foreach (var bankTransferItem in _ViewModel.EmployerBankTransferItems)
                {
                    //document header
                    AddDocHeader();

                    foreach (var _bankTransferItem in bankTransferItem.EmployerBankTransferItems)
                    {
                        //document body 
                        AddDocBody(bankTransferItem);
        ...
        #region Body
        //document body
        private void AddDocBody(EmployerBankTransferItem2 _EmployerBankTransferItems)
        {
            try
            {
                foreach (var _bankTransferItem in _EmployerBankTransferItems.EmployerBankTransferItems)
                {
                    AddBankTransferItem(_bankTransferItem);
                }
            }
            catch (Exception ex)
            {
        ...
        //table header
        private void BodyAddtableHeaders(Table bankTransferTable)
        {
            try
            {
