...
{
    public class P9BPDFBuilder
    {
        P9BReportModel _ViewModel;
        Document document;
        string Message;
        string sFilePDF;
        ...
        private void AddDocHeaders()
        {
            ...
            empInfoTable.Padding = 1;
            empInfoTable.Spacing = 1;
        ...
        private void AddDocBody()
        {
            try
            {
                Table taxTable = new Table(14, 16);
                taxTable.Padding = 1;
                taxTable.Spacing = 1;
                ...
                for (int i = 1; i <= 12; i++)
                {
                    var monthRecs = (from mr in _ViewModel.P9BEmpList
                                     where mr.MonthInt == i
                                     select mr).SingleOrDefault();
                    AddTableDetails(i, taxTable, monthRecs);
                }
        ...
        private void AddTableHeaders(Table aTable)
        {
            ...
            //Col 5
            Cell dCell = new Cell(new Phrase("Defined Contribution/\nBenefit Calculation\nKshs", tHFont));
            dCell.Colspan = 3;
            aTable.AddCell(dCell);
            aTable.AddCell(new Phrase("Defined con/Ben\nKshs", tHFont)); //Col 6
            ...
            aTable.AddCell(new Phrase("J", tHFont)); //Col 10
            aTable.AddCell(new Phrase("", tHFont)); //Col 11
            aTable.AddCell(new Phrase("K", tHFont)); //Col 12
            ...
            Cell JCell = new Cell(new Phrase("Total", tHFont)); //Col 5
            JCell.Colspan = 2;
            aTable.AddCell(JCell);
        ...
        private void AddTotals(Table aTable)
        {
            ...
            Cell K = new Cell(new Phrase(string.Format(System.Globalization.CultureInfo.InvariantCulture, "{0:N0}", _ViewModel.Total_K), tcFont));
            K.HorizontalAlignment = Cell.ALIGN_RIGHT;
        ...
        private void AddDocFooter()
        {
            ...
            Cell f21 = new Cell(new Phrase("TOTAL CHARGEABLE PAY (COL.G) Kshs." + string.Format(System.Globalization.CultureInfo.InvariantCulture, "{0:N0}", _ViewModel.Total_G.ToString("#,##0")), rms8Bold));
            f21.HorizontalAlignment = Cell.ALIGN_LEFT;
            f21.Border = Cell.NO_BORDER;
            aTable.AddCell(f21);
            Cell f22 = new Cell(new Phrase("TOTAL TAX (COL.L) Kshs." + string.Format(System.Globalization.CultureInfo.InvariantCulture, "{0:N0}", _ViewModel.Total_K.ToString("#,##0")), rms8Bold));
            ...
            sb.Append("Use P9B where the director/employee recieves Tax-free Remuneration. ");
            sb.AppendLine("\n(See back of this card for further information required by the Department)");
            sb.AppendLine("P9B");
            p2.Add(new Phrase(sb.ToString(), rms8Bold));
